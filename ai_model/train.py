...
for entry in data:
    player_position = list(
        entry["player_position"].values()
    )  # Convert the player position dictionary to a list of values
    enemy_position = list(
        entry["enemy_position"].values()
    )  # Convert the enemy position dictionary to a list of values
    inputs.append(
        player_position + enemy_position
    )  # Concatenate player and enemy positions as input features
    outputs.append(
        entry["distance"]
    )  # Use distance between player and enemy as the output label

# Convert input and output lists to PyTorch tensors
inputs_tensor = torch.tensor(
    inputs, dtype=torch.float32
)  # Convert input features to a tensor of type float32
outputs_tensor = torch.tensor(outputs, dtype=torch.float32).view(
    -1, 1
)  # Convert output labels to a tensor of type float32 and reshape to match model output size

# Define model, loss function, and optimizer
input_size = len(
    inputs[0]
)  # Determine the size of the input layer based on the length of the first input
hidden_size = 64  # Define the number of hidden units in the hidden layer
output_size = (
    1  # Define the size of the output layer, which is 1 for predicting distance
)

model = EnemyAIModel(
    input_size, hidden_size, output_size
)  # Instantiate the EnemyAIModel with the specified sizes
...
for epoch in range(epochs):  # Loop through each epoch
    ...
    if (epoch + 1) % 100 == 0:
        ...
        )  # Print the epoch number and current loss value
...
save_dir = "ai_model/saved_models"
os.makedirs(save_dir, exist_ok=True)

# Save the model
torch.save(model.state_dict(), f"{save_dir}/enemy_ai_model.pth")
